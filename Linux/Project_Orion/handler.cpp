...
protected:
  ...
  const std::vector<std::string> m_commands = {"track", "endsession", "help",
                                               "stop"};

public:
  std::vector<std::string> Handle(std::vector<std::string> request) override {
    // is request[0] a command? if no -> throw an error
    if (find(m_commands.begin(), m_commands.end(), request[0]) ==
        m_commands.end()) {
...
public:
  std::vector<std::string> Handle(std::vector<std::string> request) override {
    ...
    if (request[0] == m_commands[0]) {
      ...
      if (request.size() < 2 ||
          request.size() > 2) { // no arguments or more than needed? it's sad...
        s_message = "[Usage]: track <satellite[name]>";
        m_message = input_handler(s_message);
        delete sat0;
        return m_message;
      } else if (std::find(m_satellites.begin(), m_satellites.end(),
                           request[1]) !=
                 m_satellites.end()) { // do we have this satellite within our
                                       // m_satellites vector?

        m_latitude = latitudecal();   // getting latitude
        m_longitude = longitudecal(); // getting longitude
        // xAxisProjection&yAxisProjection return value is twice as big as we
        // need, so we simply need to multiply it by 0.00000005
        m_pixLatitude = yAxisProjection(m_latitude) *
                        0.00000005; // if latitude > 85 or latitude < -85 it may
                                    // cause -nan(ind)
        m_pixLongitude = xAxisProjection(m_longitude) *
                         0.00000005; // if longitude > 179 or longitude < -179,
                                     // then pixLongitude > 1.0 and pixLongitude
                                     // < -1.0 respectively
        // pixlatitude and pixlongitude are valid only when they are within the
        // range of [-1.0; 1.0] (including -1.0 and 1.0)
        if ((m_pixLongitude > 1.0f || m_pixLatitude > 1.0f) ||
            (m_pixLongitude < -1.0f || m_pixLatitude < -1.0f)) {
          isnan(m_pixLatitude) || isnan(m_pixLongitude)
              ? s_message = "[Error]: Either latitude or longitude is NaN"
              : s_message =
                    "[Error]: Either latitude or longitude is out of range";
          m_message = input_handler(s_message);
        } else {
          displayInitWindow = false;
          // if a satellite's name is valid -> add it to a vector and switch
          // openGL to next sceen
          m_colorAtIndex = randomColor();
          m_mtx0.lock();
          s_message = sat0->addToArray(
              satelliteStorage, request[1], {m_pixLongitude, m_pixLatitude},
              m_colorAtIndex, {m_longitude, m_latitude});
          m_message = input_handler(s_message);
          m_mtx0.unlock();
        }
        delete sat0;
  ...
  std::vector<std::string> Handle(std::vector<std::string> request) override {
    ...
    if (request[0] == m_commands[2]) {
      if (request.size() < 2 || request.size() > 2) {
        ...
        s_message =
            "List of available commands during session:\n"
    ...
    if (request[0] == m_commands[3]) {
      if (request.size() < 2 || request.size() > 2) {
        ...
        if (std::find(m_satellites.begin(), m_satellites.end(), request[1]) !=
          ...
          satelliteStorage.empty() ? displayInitWindow = true
                                   : displayInitWindow = false;
          m_message = input_handler(s_message);
          m_mtx0.unlock();
          delete sat1;
